...
function Tasks() {
  ...
  const searchChange = () => {
    axios
      .get(`https://rocky-temple-83495.herokuapp.com/tasks?name_like=${search}`)
      .then((res) => {
        const persons = res.data;
        if (persons.length == 0) {
          axios
            .get(
              `https://rocky-temple-83495.herokuapp.com/tasks?description_like=${search}`
            )
            .then((res) => {
              const persons = res.data;
              if (persons.length == 0) {
                axios
                  .get(
                    `https://rocky-temple-83495.herokuapp.com/tasks?startDate=${search}`
                  )
                  .then((res) => {
                    const persons = res.data;
                    if (persons.length == 0) {
                      axios
                        .get(
                          `https://rocky-temple-83495.herokuapp.com/tasks?endDate=${search}`
                        )
                        .then((res) => {
                          const persons = res.data;
                          setdata2(persons);
  ...
  return (
    <>
      ...
      <div className="d-flex" role="search" style={{ width: "500px", margin:"auto" , marginTop:"50px", marginBottom:"50px" }}>
        <input
